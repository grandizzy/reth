    ...
    TxEnv: FromRecoveredTx<TransactionSigned> + FromTxWithEncoded<TransactionSigned>,
{
    ...
    fn commit_transaction(&mut self, output: Self::Result) -> GasOutput {
        // Apply any pending segment boundary BEFORE committing this tx. Each segment boundary
        // runs `inner.finish()` for the just-completed segment (post-execution system calls,
        // withdrawals) and `apply_pre_execution_changes` for the new segment (EIP-2935 history
        // storage, EIP-4788 beacon roots). Without this, BEACON_ROOTS / HISTORY_STORAGE only
        // reflect the FIRST sub-block's writes and subsequent sub-blocks' state lookups (e.g.
        // EIP-4788 timestamp-keyed reads) miss values that mainnet had — causing divergent
        // execution downstream that surfaces as `SlotNotFound`. Pre-bump this was wired in via
        // `commit_transaction`'s Result return; the new alloy-evm trait returns `GasOutput`
        // unconditionally, so a boundary-apply failure here aborts via expect.
        self.maybe_apply_boundary().expect("segment boundary apply failed");
